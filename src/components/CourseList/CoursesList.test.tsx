...
describe('CoursesList Component', () => {
  ...
  test('renders empty message when no courses are provided', () => {
    renderWithRouter(<CoursesList courses={[]} />);
  ...
  test('renders custom empty message when provided', () => {
    ...
    renderWithRouter(<CoursesList courses={[]} emptyMessage={customMessage} />);
    expect(screen.getByText(customMessage)).toBeInTheDocument();
  });

  test('renders course cards when courses are provided', () => {
    renderWithRouter(<CoursesList courses={mockCourses} />);
  ...
  test('displays course level with correct badge class', () => {
    const { container } = renderWithRouter(<CoursesList courses={mockCourses} />);
  ...
  test('displays correct level with correct badge class for intermediate courses', () => {
    const mockData: Course[] = [{ ...mockCourses[1], _id: '3', level: 'Intermediate' }];
    const { container } = renderWithRouter(<CoursesList courses={mockData} />);
